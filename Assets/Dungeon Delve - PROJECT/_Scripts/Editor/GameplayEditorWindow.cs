...
public class GameplayEditorWindow : EditorWindow
{
    ...
    {
        _questSetups = Resources.LoadAll<QuestSetup>("Quest Setup");
        ...
        if (_questSetups != null && _questSetups.Any())
        {
            ...
            foreach (var questSetup in _questSetups)
            {
                ...
                GUILayout.Label( "Has Item Requirement", Width(150));
                GUILayout.Label( "Item Needed", Width(150));
                GUILayout.Label( "Item Value", Width(150));
                ...
                var _taskRequirement = questSetup.GetRequirement();
                _taskRequirement.SetUseRequirement(EditorGUILayout.Toggle("", _taskRequirement.IsUseRequirement, Width(20)));

                if (_taskRequirement.IsUseRequirement)
                {
                    GUILayout.Label( "", Width(130));
                    _taskRequirement.SetNameCode((ItemNameCode)EditorGUILayout.EnumPopup("", _taskRequirement.code, GUILayout.Width(150)));
                    _taskRequirement.SetValue(EditorGUILayout.IntField("", _taskRequirement.value, GUILayout.Width(150)));
                }
                GUILayout.EndHorizontal();
        ...
        if (GUILayout.Button("+", Width(45), Height(25)))
        {
            var _path = $"Assets/Resources/Quest Setup/Quest_{_count + 1}.asset";
        ...
        if (GUILayout.Button("-", Width(45), Height(25)))
        {
            var _path = $"Assets/Resources/Quest Setup/Quest_{_count}.asset";
