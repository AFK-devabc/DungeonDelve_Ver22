...
public class GoblinSlingshot_Effects : MonoBehaviour, IAttack
{
    ...
    public void EffectAttack()
    {
        ...
        var playerPos = enemyController.Blackboard.GetVariable<GameObject>("Player");
        var _taget = playerPos.value.transform.position;
        _taget.y += Random.Range(1f, 1.5f);
        var rotation = Quaternion.LookRotation(_taget - effectPoint.position);
